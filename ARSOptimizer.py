...
class ARSOptimizer(Optimizer):
    def __init__(self, parameters: Tensor, get_env, action_sz: int, get_policy: Callable[[Tensor, Normalizer], Any],
                 step_sz=1E-2, sdv=1E-3, n_directions=50, n_choice=None, hrz=1, normalizer=None):
    ...
    def ars_step(self, Horizon: int, n_choice: int, n_directions: int, parameters: Tensor, sdv: float, step_sz: float):
        ...
        rollouts_plus: List[List[tuple]] = [self.query_oracle(param, Horizon) for param in params_plus]
        rollouts_minus: List[List[tuple]] = [self.query_oracle(param, Horizon) for param in params_minus]
    ...
    def query_oracle(self, params: Tensor, horizon: int) -> List[tuple]:
